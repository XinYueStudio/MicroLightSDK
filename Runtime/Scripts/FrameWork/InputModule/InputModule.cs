...
namespace MicroLight
{
    ...
    using UnityPlugin.Utility;
    using UnityPlugin.Pointer3D;
    using System;
    public delegate void UIPointerHandler(RaycastResult Result);

    [AddComponentMenu(" MicroLight/InputModule"), DisallowMultipleComponent]
    public class InputModule : BaseInputModule
    {
        ...
        public List<Pointer3DRaycaster> processingRaycasters = new List<Pointer3DRaycaster>();
        ...
        public bool ShowLineRender = false;
        public bool ShowPointer = true;
        public float farDistance = 500;
        ...
        public static void Initialize()
        {
            ...
            var instances = FindObjectsOfType<InputModule>();
            ...
            if (!Active)
            {
                ...
                instance = eventSystem.gameObject.AddComponent<InputModule>();
            }
        ...
        public event UIPointerHandler PointerOnCursorLayer;
        public event UIPointerHandler PointerUnOnCursorLayer;
        public LayerMask CursorLayer = -1;

        //[SerializeField]
        private GameObject _FocusPointer;
        public GameObject FocusPointer
        {
            get
            {
                ...
                {
                    _FocusPointer = new GameObject("FocusPointer");
                }

                return _FocusPointer;
            }
            set
            {
                _FocusPointer = value;
           ...
           private Vector3 FocusPointerVector;
        ...
        {
          //  Debug.Log("raycasters：" + raycasters.Count);
            ...
            if (processingRaycasters.Count > 0)
            {
                for (var i = processingRaycasters.Count - 1; i >= 0; --i)
                {
                    ...
                    if (result.gameObject != null)
                    {
                        ...
                        int layer = CursorLayer.value | (1 << result.gameObject.layer);

                        if (layer == CursorLayer.value)
                        {
                            // prepare raycaster value
                            scrollDelta = raycaster.GetScrollDelta();
                            ...
                            if (!hoverEventData.pointerPress && !Input.GetKey(KeyCode.Mouse0))
                            ...
                            if (rectTransform)
                            {
                                ...
                                FocusPointer.transform.parent = result.gameObject.transform;
                                FocusPointer.transform.position = result.worldPosition;


                                if (FocusPointerVector != FocusPointer.transform.position)
                                {
                                    FocusPointerVector = FocusPointer.transform.position;
                                    Vector2 POS = new Vector2(FocusPointer.transform.localPosition.x, FocusPointer.transform.localPosition.y);
                                    Vector2 Vector = new Vector2((float)((rectTransform.rect.width * 0.5f + POS.x) / rectTransform.rect.width),
                                                    (float)((rectTransform.rect.height * 0.5f + POS.y) / rectTransform.rect.height));


                                    var moveHandler = ExecuteEvents.GetEventHandler<IMoveHandler>(result.gameObject);
                                    var axisEventData = new AxisEventData(this.eventSystem);
                                    axisEventData.moveVector = Vector;


                                    ExecuteEvents.ExecuteHierarchy(moveHandler, axisEventData, ExecuteEvents.moveHandler);
                            ...
                            else if (result.isValid && CanScroll && scrollDelta == Vector2.zero)
                            {
                                CanScroll = false;
                                var scrollHandler = ExecuteEvents.GetEventHandler<IScrollHandler>(result.gameObject);
                                ExecuteEvents.ExecuteHierarchy(scrollHandler, hoverEventData, ExecuteEvents.scrollHandler);
                            ...
                            if (PointerOnCursorLayer != null)
                            {
                                PointerOnCursorLayer(result);
                            }
                        ...
                        {
                            if (PointerUnOnCursorLayer != null)
                            {
                                PointerUnOnCursorLayer(result);
                            }
                        }
                    }
                    else
            ...
            else
            {
                if (PointerUnOnCursorLayer != null)
                {
                    PointerUnOnCursorLayer(new RaycastResult());
                }
            }
