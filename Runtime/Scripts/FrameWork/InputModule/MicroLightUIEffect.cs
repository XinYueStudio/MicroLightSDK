...
{
    public class MicroLightUIEffect : BaseMeshEffect
    {
        ...
        void Update()
        {
            ...
            if (!tesselationRequired && !curvingRequired) // do not perform a check if we're already tesselating or curving. Tesselation includes curving.
            {
                ...
                if (!testedPos.AlmostEqual(savedPos))
                {
                    ...
                    if (Mathf.Pow(testedPos.x - savedPos.x, 2) > 0.00001F || Mathf.Pow(testedPos.z - savedPos.z, 2) > 0.00001F)
                    {
                        savedPos = testedPos;
        ...
        void ModifyVerts(List<UIVertex> verts)
        {
            ...
            angle = mySettings.Angle;
            //float ret = ((transform as RectTransform).rect.size.x * 0.5f) / Mathf.Sin(Mathf.Clamp(angle, -180.0f, 180.0f) * 0.5f * Mathf.Deg2Rad);

            float radius =   ((myCanvas.transform as RectTransform).rect.size.y / ((2 * Mathf.PI) * (angle / 360.0f)));
        ...
        void TesselateGeometry(List<UIVertex> verts)
        {
            ...
            Vector2 canvasSize = myCanvas.transform.GetComponent<RectTransform>().rect.size;
            ...
            if (angle != 0)
            {
                ret = Mathf.Min(canvasSize.x / 4, canvasSize.x / (Mathf.Abs(angle).Remap(0.0f, 360.0f, 0, 1) * mySettings.baseCircleSegments));
                ret2 = Mathf.Min(canvasSize.y / 4, canvasSize.y / (Mathf.Abs(angle).Remap(0.0f, 360.0f, 0, 1) * mySettings.baseCircleSegments));
            }
            Vector2 tessellatedSize = new Vector2(ret, ret2) /Mathf.Clamp(mySettings.Quality, 0.01f, 10.0f);
